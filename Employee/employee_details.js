...
function findBySpecialization(lang) {
  const result = employees.filter(
    (employee) => employee.specialization === lang,
  );
  ...
  if (result) {
    document.getElementById("employeesDetails").innerHTML = result
      .map(
        (foundEmployee) =>
          `${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary} - ${foundEmployee.specialization} </p>`,
      )
      .join("");
  } else {
    document.getElementById("employeesDetails").innerHTML =
      "Do not have any employee satify this condition";
  }
}
